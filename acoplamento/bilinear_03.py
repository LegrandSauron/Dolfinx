...
(u, T) = ufl.split(w) #funcao incognita
(u0,T0)= ufl.split(w0) #condiçao inicial para os steps
...
q_grd = fem.Constant(domain, PETSc.ScalarType(0.0))
...
def deltaT(temp,temp0):
    return temp-temp0

def sigma(u,temp,temp0):
    return lmbda * ufl.tr(epsilon(u)) * I(u) + 2*mu*epsilon(u) - alpha*deltaT(temp,temp0)* ufl.Identity(2)
...
F = ufl.inner(ufl.grad(u_test), sigma(u,T,T0))*ufl.dx + ufl.inner(u_test,u)*ufl.dx + dt*ufl.inner(ufl.grad(T_test),ufl.grad(T))*k*ufl.dx

F += ufl.inner(u_test, loading)*ufl.dx - ufl.inner(u_test, Traction)*ds(2) +  dt*ufl.inner(T_test,q_grd)*ufl.dx + ufl.inner(T_test,T_init)*ufl.dx


"""Solucao"""
problem = fem.petsc.NonlinearProblem(F, w, bc)
...
for i in range(steps):
    ...
    w0.x.array[:] = w.x.array
    xdmf.write_function(w.sub(1), tempo_init)
