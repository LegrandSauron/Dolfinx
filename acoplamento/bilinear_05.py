...
def sigma(u,Temp):
    return (ufl.tr(epsilon(u)) * I(u) + epsilon(u) ) - alpha*Temp * I(u)
...
a = ufl.inner(ufl.grad(u_test), sigma(u,Temperatura))*ufl.dx + ufl.inner(u_test,u)*ufl.dx + ufl.inner(ufl.grad(T_test),ufl.grad(Temperatura))*ufl.dx

#L = ufl.inner(u_test, loading)*ufl.dx - ufl.inner(u_test, Traction)*ds() +  dt*ufl.inner(T_test,q)*ufl.dx + ufl.inner(T_test,T_init)*ufl.dx
...
for i in range(steps):
    t_init += dt
    uh 	  = problem.solve()
    Uf= uh.sub(0).collapse()
    Tf = uh.sub(1).collapse() 
    
    T_init.interpolate(Tf)#Atualizando os valores de temperatura novos para serem os antigos do passo posterior 
    xdmf.write_function(Uf,t_init)