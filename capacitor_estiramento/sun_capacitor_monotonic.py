...
bcs_f = DirichletBC(ME.sub(2),phiRamp,facets,6) # Ramp up phi on top face aplicaçao da voltagem

#bcs_b1 = DirichletBC(ME.sub(0),Constant((0.,0., 0.)),facets,2) # left face built-in
bcs_b1 = DirichletBC(ME.sub(0).sub(0),0.0,facets,2) # pull right face engaste
bcs_b2 = DirichletBC(ME.sub(0).sub(0),disp,facets,7) # pull right face Estiramento
bcs_a = DirichletBC(ME.sub(0).sub(0),0.,facets,2) #Igual a bcs_b1, com exceção de que é 0. e nao 0.0

bcs_b3 = DirichletBC(ME.sub(0).sub(1),0.0,facets,3) # nao há variação no eixo z, o corpo nao afina
bcs_b4 = DirichletBC(ME.sub(0).sub(2),0.0,facets,5) #O que é ?
