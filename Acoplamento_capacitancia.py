...
domain, cell_tags, facet_tags = gmshio.read_from_msh("capacitormalha.msh", MPI.COMM_SELF,0, gdim=2)
...
Init_CPos.x.array[material_[0]]= np.log(DOlfin_Eps)
Init_CPos.x.array[material_[1]]= np.log(cPos0)
Init_CPos.x.array[material_[2]]= np.log(cPos0)
w_old.sub(1).interpolate(Init_CPos)

Init_CNeg= fem.Function(Q)
Init_CNeg.x.array[material_[0]]= np.log(DOlfin_Eps)
Init_CNeg.x.array[material_[1]]= np.log(cNeg0)
Init_CNeg.x.array[material_[2]]= np.log(cNeg0)
