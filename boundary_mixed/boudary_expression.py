...
class disp_Exp:
    ...
    def eval(self,x):
        #return 5.5*self.scalex*self.t/self.Tramp
        return np.stack((np.zeros(x.shape[1]), np.full(x.shape[1], 5.5*self.scalex*self.t/self.Tramp, dtype=ScalarType)))
...
dispV.Tramp= T2_tot
dispV.scalex= scaleX
disp, _, __,___ = fem.Function(ME).split()
